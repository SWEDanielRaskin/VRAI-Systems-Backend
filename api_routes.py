...
STRIPE_CONNECT_CLIENT_SECRET = os.getenv('STRIPE_CONNECT_CLIENT_SECRET')
STRIPE_CONNECT_REDIRECT_URI = os.getenv('STRIPE_CONNECT_REDIRECT_URI', 'https://vraisystems.up.railway.app/api/oauth/stripe/callback')

# Stripe Connect scopes
STRIPE_CONNECT_SCOPES = 'read_write'  # Standard Stripe Connect scope
...
@api_bp.route('/calendar/embed-url', methods=['GET'])
@require_auth
def get_calendar_embed_url():
    """Get Google Calendar embed URL for the selected calendar"""
    try:
        calendar_data = db.get_selected_calendar('default')
        ...
        logger.error(f"Error getting calendar embed URL: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
@api_bp.route('/oauth/stripe/auth', methods=['GET'])
@require_auth
def stripe_oauth_auth():
    ...
    try:
        ...
        # Build Stripe Connect authorization URL
        auth_params = {
            'response_type': 'code',
        ...
        authorization_url = 'https://connect.stripe.com/oauth/authorize?' + urllib.parse.urlencode(auth_params)
        
        logger.info(f"🔗 Stripe Connect OAuth initiated for state: {state}")
        ...
        logger.error(f"Error initiating Stripe OAuth: {str(e)}")
        return jsonify({'error': str(e)}), 500

@api_bp.route('/oauth/stripe/callback', methods=['GET'])
def stripe_oauth_callback():
    ...
    try:
        ...
        if error:
            logger.error(f"Stripe OAuth error: {error}")
            return redirect('https://vraisystems.org/oauth-error.html?error=stripe_oauth_failed')
        
        if not code:
            logger.error("No authorization code received from Stripe")
            return redirect('https://vraisystems.org/oauth-error.html?error=no_code')
            
        if not STRIPE_CONNECT_CLIENT_ID or not STRIPE_CONNECT_CLIENT_SECRET:
            logger.error("Stripe Connect credentials not configured")
            return redirect('https://vraisystems.org/oauth-error.html?error=not_configured')
        ...
        logger.info(f"🔄 Exchanging Stripe authorization code for access token...")
        
        response = requests.post('https://connect.stripe.com/oauth/token', data=token_data)
        
        if response.status_code != 200:
            logger.error(f"Stripe token exchange failed: {response.status_code} - {response.text}")
            return redirect('https://vraisystems.org/oauth-error.html?error=token_exchange_failed')
        ...
        if not access_token or not stripe_user_id:
            logger.error("Invalid token response from Stripe")
            return redirect('https://vraisystems.org/oauth-error.html?error=invalid_token')
        
        # Store credentials in database
        expires_at = int(time.time()) + (365 * 24 * 60 * 60)  # Stripe tokens don't expire, set to 1 year
        
        db.store_oauth_credentials(
            provider='stripe',
        ...
        logger.info(f"✅ Stripe Connect successful for user: {stripe_user_id}")
        
        # Redirect to success page
        return redirect('https://vraisystems.org/oauth-success.html?provider=stripe')
        
    except Exception as e:
        logger.error(f"Error in Stripe OAuth callback: {str(e)}")
        return redirect('https://vraisystems.org/oauth-error.html?error=callback_error')

@api_bp.route('/oauth/stripe/status', methods=['GET'])
@require_auth
def stripe_oauth_status():
    """Check Stripe Connect OAuth status"""
    try:
        credentials_data = db.get_oauth_credentials('stripe', 'default')
        ...
        # Check if token is still valid (Stripe tokens don't expire, but check anyway)
        current_time = int(time.time())
        if credentials_data['expires_at'] and current_time > credentials_data['expires_at']:
            return jsonify({
                'connected': False,
                'message': 'Stripe connection expired'
            })
        
        # Verify token with Stripe API
        try:
            stripe.api_key = credentials_data['access_token']
            ...
            logger.warning("Stripe token is invalid, removing from database")
            db.delete_oauth_credentials('stripe', 'default')
            return jsonify({
                'connected': False,
                'message': 'Stripe connection invalid'
            })
        
    except Exception as e:
        logger.error(f"Error checking Stripe OAuth status: {str(e)}")
        return jsonify({'error': str(e)}), 500

@api_bp.route('/oauth/stripe/disconnect', methods=['POST'])
@require_auth
def stripe_oauth_disconnect():
    """Disconnect Stripe Connect OAuth"""
    try:
        credentials_data = db.get_oauth_credentials('stripe', 'default')
        
        if not credentials_data:
            return jsonify({'message': 'Not connected to Stripe'})
        
        # Revoke the token with Stripe (optional - tokens remain valid until manually revoked)
        try:
            stripe.api_key = credentials_data['access_token']
            # Note: Stripe doesn't have a direct revoke endpoint for Connect tokens
            # The disconnect happens when we remove from our database
            logger.info("🔓 Revoking Stripe Connect access...")
        except Exception as e:
            logger.warning(f"Could not revoke Stripe token: {str(e)}")
        
        # Remove credentials from database
        db.delete_oauth_credentials('stripe', 'default')
        
        logger.info("✅ Stripe Connect disconnected successfully")
        ...
        logger.error(f"Error disconnecting Stripe OAuth: {str(e)}")
        return jsonify({'error': str(e)}), 500
